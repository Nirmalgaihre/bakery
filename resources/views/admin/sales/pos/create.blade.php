...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function calculateTotals() {
        ...
        cart.forEach((item, index) => {
            ...
            if (rows[index]) {
                rows[index].querySelector('.item-total-lbl').textContent = `NPR ${itemTotal.toFixed(2)}`;
            }
        });
    ...
    submitBtn.addEventListener('click', function () {
        ...
        const payload = {
            ...
            transaction_date : document.getElementById('transaction-date') ? document.getElementById('transaction-date').value.trim() : '',
            items: cart.map(item => ({
                id          : item.id,
        ...
        .then(result => {
            if (result.status === 200 && result.data.success) {
                ...
                if (!alertContainer) {
                    ...
                    alertContainer.className = "fixed top-5 right-5 z-50 flex flex-col gap-4 w-full max-w-sm pointer-events-none";
