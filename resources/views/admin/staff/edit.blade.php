...
<div class="container mx-auto py-6 px-4 max-w-3xl font-sans antialiased text-slate-600">
    ...
    <div class="mb-6">
        <h1 class="text-lg font-bold text-slate-900 tracking-tight">Modify Staff Operator Profile</h1>
        <p class="text-xs text-slate-400 mt-0.5">Update system access parameters, credentials, or administrative role groups for this employee.</p>
    </div>

    <div class="bg-white border border-slate-200/60 rounded-lg shadow-sm overflow-hidden p-6">
        <form method="POST" action="{{ route('admin.staff.update', $staff->id) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div>
                <label for="name"
                    class="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">Full Name <span
                        class="text-rose-500">*</span></label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-slate-400">
                        <i class="fa-solid fa-user text-xs"></i>
                    </span>
                    <input type="text" id="name" name="name" value="{{ old('name', $staff->name) }}" required autofocus
            ...
            <div>
                <label for="email"
                    class="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">Email Address
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="password"
                        class="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">New Password <span class="text-slate-400 font-normal lowercase">(Leave blank to keep current)</span></label>
                    <div class="relative">
                        <span
                ...
                <div>
                    <label for="password_confirmation"
                        class="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">Confirm New Password</label>
                    <div class="relative">
                        <span
                            class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-slate-400">
            ...
            <div>
                <label for="role"
                    class="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wider">Assign Baseline
                ...
                <div class="relative">
                    ...
                    <select id="role" name="role" required
                        ...
                        <option value="admin" {{ old('role', $staff->roles->first()?->name ?? '') == 'admin' ? 'selected' : '' }}>Administrator</option>
                        <option value="accountant" {{ old('role', $staff->roles->first()?->name ?? '') == 'accountant' ? 'selected' : '' }}>Accountant Hub</option>
                    </select>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit"
                    ...
                    <i class="fa-solid fa-cloud-arrow-up text-[11px]"></i> Update Operator Profile
                </button>
            </div>
        </form>
    </div>
