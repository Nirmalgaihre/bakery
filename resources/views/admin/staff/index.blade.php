...
<div class="container mx-auto py-6 px-4 max-w-7xl font-sans antialiased text-slate-600">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-xs text-slate-400 mt-0.5">Manage administrative accounts, operators, and assign baseline privileges.</p>
        </div>
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    <td class="py-3 px-4">
                        <div class="flex items-center gap-3">
                            <div class="h-7 w-7 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600 font-bold uppercase text-[11px]">
                    ...
                    <td class="py-3 px-4">
                        @if(method_exists($staff, 'getRoleNames') && $staff->getRoleNames()->count() > 0)
                            @foreach($staff->getRoleNames() as $role)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] bg-blue-50 text-blue-700 border border-blue-100 font-semibold uppercase tracking-wide">
                                    {{ $role }}
                                </span>
                            @endforeach
                        @else
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] bg-slate-100 text-slate-600 font-medium">
                                Standard Operator
                            </span>
                        @endif
                    </td>
                    
                    <td class="py-3 px-4 font-mono text-slate-500">{{ $staff->created_at ? $staff->created_at->format('Y-m-d H:i') : 'N/A' }}</td>
                    ...
                    <td class="py-3 px-4 text-center space-x-1.5">
                        ...
                        <form action="{{ route('admin.staff.destroy', $staff->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to completely terminate this operator access profile?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
