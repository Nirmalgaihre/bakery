...
<div class="max-w-7xl w-full mx-auto">
    ...
    <div class="bg-white border border-slate-200 rounded-lg shadow-xs overflow-hidden mt-4">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.products.export', ['type' => 'xlsx']) }}"
                class="inline-flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[11px] px-3 py-2 rounded uppercase tracking-wide shadow-xs">
                <i class="fa-solid fa-file-excel"></i> Export Excel
            </a>
            <a href="{{ route('admin.products.export', ['type' => 'csv']) }}"
                class="inline-flex items-center gap-1.5 bg-slate-600 hover:bg-slate-700 text-white font-bold text-[11px] px-3 py-2 rounded uppercase tracking-wide shadow-xs">
                <i class="fa-solid fa-file-csv"></i> Export CSV
            </a>
            <a href="{{ route('admin.products.import.form') }}"
                class="inline-flex items-center gap-1.5 bg-amber-600 hover:bg-amber-700 text-white font-bold text-[11px] px-3 py-2 rounded uppercase tracking-wide shadow-xs">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-sm text-slate-700">
                <thead>
                    <tr
                        class="bg-slate-100/70 border-b border-slate-200 text-xs font-bold uppercase tracking-wider text-slate-500">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="p-4">
                            <span
                                class="bg-slate-100 text-slate-700 text-xs px-2.5 py-1 rounded-full font-medium border border-slate-200/50">{{ $product->category }}</span>
                        </td>
                        <td class="p-4 text-right font-mono text-xs text-slate-600">Rs.
                            {{ number_format($product->purchase_cost, 2) }}</td>
                        <td class="p-4 text-right font-mono text-xs font-bold text-slate-900">Rs.
                            {{ number_format($product->selling_price, 2) }}</td>
                        <td class="p-4 text-center">
                            <span
                                class="uppercase font-mono text-[11px] bg-slate-100 text-slate-600 border border-slate-200 px-2 py-0.5 rounded">{{ $product->inventory_unit }}</span>
                        </td>
                        ...
                        <td class="p-4 text-center">
                            @if($totalAvailable <= $product->alert_stock_level)
                                <span
                                    class="bg-rose-50 text-rose-700 border border-rose-200 text-[10px] font-bold uppercase px-2 py-0.5 rounded animate-pulse">Low</span>
                                @else
                                <span
                                    class="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold uppercase px-2 py-0.5 rounded">Good</span>
                                @endif
                        </td>

                        <td class="p-4 text-right px-5">
                            <a href="{{ route('admin.inventory.create', $product->id) }}"
                                class="inline-flex items-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[11px] px-2.5 py-1 rounded uppercase shadow-xs">
                                Add
                            </a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="10" class="p-8 text-center text-sm text-slate-400">No records found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
