            ...
            <table class="w-full text-left border-collapse text-sm text-slate-700">
                <thead>
                    <tr class="bg-slate-100/70 border-b border-slate-200 text-xs font-bold uppercase tracking-wider text-slate-500">
                        ...
                        <th class="p-4 text-center">Packaging Unit</th>
                        <th class="p-4 text-center">Stock Available</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($products as $product)
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td class="p-4 text-center">
                                <span class="uppercase font-mono text-xs bg-blue-50 text-blue-700 border border-blue-100 px-2 py-0.5 rounded">
                                    {{ $product->inventory_unit }}
                                </span>
                            </td>
                            <td class="p-4 text-center font-semibold font-mono">{{ $product->initial_stock }}</td>
                            <td class="p-4 text-center">
                            ...
                            <td class="p-4 text-right px-5 whitespace-nowrap">
                                {{-- CRITICAL CHANGE: Refurbished from Yellow to clean Emerald Green UI element controls --}}
                                <a href="{{ route('admin.stock.create', $product->id) }}" class="inline-flex items-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[11px] px-2.5 py-1 rounded transition-colors uppercase tracking-wide shadow-xs">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="p-8 text-center text-sm text-slate-400 bg-slate-50/30">
