...
<div class="max-w-4xl w-full mx-auto">
    ...
    <div id="form-skeleton" class="animate-pulse bg-white border border-slate-200 rounded-lg shadow-xs overflow-hidden">
        <div class="p-4 bg-slate-100 h-10 w-full mb-6"></div>
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div class="h-14 bg-slate-100 rounded"></div>
                <div class="h-14 bg-slate-100 rounded md:col-span-2"></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div class="h-14 bg-slate-100 rounded"></div>
                <div class="h-14 bg-slate-100 rounded"></div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 p-4 bg-slate-50 border border-slate-100 rounded-md">
                <div class="h-14 bg-slate-200/60 rounded"></div>
                <div class="h-14 bg-slate-200/60 rounded"></div>
            </div>
        </div>
    ...
    <form id="product-form" action="{{ route('admin.products.store') }}" method="POST" class="hidden bg-white border border-slate-200 rounded-lg shadow-xs overflow-hidden">
        @csrf
        ...
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                {{-- Item Code --}}
                <div class="space-y-1">
                    <label for="item_code" class="block text-xs font-bold text-slate-700 tracking-wide uppercase">Item Code *</label>
                    <input type="text" name="item_code" id="item_code" value="{{ old('item_code') }}" placeholder="e.g. DC-001" 
                        class="w-full text-sm p-2 px-3 border rounded outline-none transition-all @error('item_code') border-red-500 focus:ring-1 focus:ring-red-500 @else border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 @enderror" required>
                    @error('item_code') <p class="text-xs text-red-500 mt-0.5 font-medium">{{ $message }}</p> @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                {{-- Dynamic Sector Category Dropdown --}}
                <div class="space-y-1">
                ...
                {{-- Color --}}
                <div class="space-y-1">
                    <label for="color" class="block text-xs font-bold text-slate-700 tracking-wide uppercase">Color</label>
                    <input type="text" name="color" id="color" value="{{ old('color') }}" placeholder="e.g. Transparent" 
                ...
                {{-- Size --}}
                <div class="space-y-1">
                    <label for="size" class="block text-xs font-bold text-slate-700 tracking-wide uppercase">Size</label>
                    <input type="text" name="size" id="size" value="{{ old('size') }}" placeholder="e.g. 500ml" 
                        class="w-full text-sm p-2 px-3 border border-slate-200 rounded outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                {{-- Inventory Unit --}}
                <div class="space-y-1">
                    <label for="inventory_unit" class="block text-xs font-bold text-slate-700 tracking-wide uppercase">Inventory Unit *</label>
                    <select name="inventory_unit" id="inventory_unit" class="w-full text-sm p-2 px-3 border border-slate-200 rounded bg-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" required>
                        <option value="" disabled {{ old('inventory_unit') == '' ? 'selected' : '' }}>Choose unit...</option>
