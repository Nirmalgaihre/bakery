    ...
    <form action="{{ route('admin.products.store') }}" method="POST" class="bg-white border border-slate-200 rounded-lg shadow-xs overflow-hidden">
        ...
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {{-- Dynamic Sector Category Dropdown Selection --}}
                <div class="space-y-1">
                    <label for="category_id" class="block text-xs font-bold text-slate-700 tracking-wide uppercase">Category *</label>
                    <select name="category_id" id="category_id" class="w-full text-sm p-2 px-3 border rounded bg-white outline-none transition-all @error('category_id') border-red-500 focus:ring-1 focus:ring-red-500 @else border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 @enderror" required>
                        <option value="" disabled {{ old('category_id') == '' ? 'selected' : '' }}>Select Category From System Registry...</option>
                        @forelse($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @empty
                            <option value="" disabled class="text-rose-500 italic">No system categories found! Please create one first.</option>
                        @endforelse
                    </select>
