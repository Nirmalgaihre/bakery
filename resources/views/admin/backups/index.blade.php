...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    {{-- Alert Messages --}}
    @if(session('success'))
        <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-xs font-semibold text-emerald-800 flex items-center gap-2">
            <i class="fa-solid fa-circle-check text-emerald-600 text-sm"></i> {{ session('success') }}
        </div>
    @endif
    @if(session('error'))
        <div class="bg-rose-50 border border-rose-200 rounded-xl p-4 text-xs font-semibold text-rose-800 flex items-center gap-2">
            <i class="fa-solid fa-circle-xmark text-rose-600 text-sm"></i> {{ session('error') }}
        </div>
    @endif

    <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">Generate snapshot archives of your database files and warehouse inventories.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm space-y-6">
            ...
            <form action="{{ route('admin.backups.store') }}" method="POST" class="space-y-5">
                ...
                <div class="space-y-3">
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wide">Backup Target Scope</label>
                    
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <label class="relative flex items-start p-4 rounded-xl border border-blue-100 bg-blue-50/30 cursor-pointer hover:bg-blue-50/50 transition-colors">
                            <div class="flex items-center h-5">
                                <input type="radio" name="backup_scope" value="all" checked class="h-4 w-4 text-blue-600 border-slate-300 focus:ring-blue-50">
                            </div>
                            <div class="ml-3">
                                <span class="block text-xs font-bold text-slate-800">Complete System Backup</span>
                                <span class="block text-[11px] text-slate-500 mt-0.5">Database tables, ledger logs, and media assets.</span>
                            </div>
                        </label>
                        ...
                        <label class="relative flex items-start p-4 rounded-xl border border-slate-200 bg-white cursor-pointer hover:bg-slate-50/50 transition-colors">
                            ...
                            <div class="ml-3">
                                ...
                                <span class="block text-[11px] text-slate-500 mt-0.5">Lightweight dump of inventories and sales ledgers.</span>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    <label for="prefix" class="block text-xs font-bold text-slate-700 uppercase tracking-wide">Custom Filename Prefix (Optional)</label>
                ...
                <div class="pt-2 border-t border-slate-100 flex items-center justify-between">
                    <div class="flex items-center gap-2 text-amber-600 bg-amber-50 rounded-md p-1.5 px-2.5 text-[10px] font-semibold">
                        <i class="fa-solid fa-triangle-exclamation"></i> May temporarily slow intensive background queries.
                    </div>
                    
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2.5 rounded-lg transition-all shadow-sm flex items-center gap-2">
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-4">
                ...
                <div class="space-y-3">
                    ...
                    <div class="space-y-1">
                        <div class="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                            <div class="bg-blue-600 h-full rounded-full" style="width: {{ $diskDetails['percentage'] }}%"></div>
                        </div>
                ...
                <div class="bg-slate-50/80 border border-slate-100 rounded-xl p-3 text-[11px] space-y-1.5 text-slate-600">
                    ...
                    <p class="font-mono text-[10px] text-slate-400">Timestamp: {{ $diskDetails['last_backup'] ? $diskDetails['last_backup']->created_at->format('Y-m-d h:i A') : 'N/A' }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="bg-slate-100 text-slate-600 text-[10px] font-mono px-2 py-0.5 rounded-md font-semibold">Total: {{ count($backups) }} Local Files</span>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 text-xs text-slate-700 font-medium">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="py-3.5 px-6 text-right space-x-1.5">
                            <a href="{{ route('admin.backups.download', $backup->filename) }}" title="Download Archive" class="text-slate-500 hover:text-blue-600 transition-colors p-1 inline-block">
                                <i class="fa-solid fa-download"></i>
                            </a>
                            
                            <form action="{{ route('admin.backups.destroy', $backup->filename) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to permanently delete this system archive snapshot?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" title="Delete Archive" class="text-slate-500 hover:text-red-600 transition-colors p-1">
