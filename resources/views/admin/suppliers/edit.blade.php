...
@section('title', 'Edit Supplier - Admin Console | Deurali Chemicals Pvt Ltd')
...
<div class="max-w-4xl w-full mx-auto">
    ...
    {{-- Interactive Supplier Modification Engine Form --}}
    <form action="{{ route('admin.suppliers.update', $supplier) }}" method="POST" class="bg-white border border-slate-200 rounded-lg shadow-xs overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-4 px-5 border-b border-slate-100 bg-slate-50/70 text-xs font-bold text-slate-600 tracking-wider uppercase flex items-center justify-between">
            <div class="flex items-center gap-2">
                <i class="fa-solid fa-pen-to-square text-blue-600"></i> Modify Supplier Details
            </div>
        ...
        <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $supplier->name) }}" placeholder="e.g. Deurali Chemicals Pvt. Ltd." 
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="contact_person" id="contact_person" value="{{ old('contact_person', $supplier->contact_person) }}" placeholder="e.g. Ram Sharma" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="space-y-1">
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $supplier->phone) }}" placeholder="e.g. 98XXXXXXXX" 
                        class="w-full text-sm p-2 px-3 border border-slate-200 rounded outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $supplier->email) }}" placeholder="e.g. contact@supplier.com" 
                        class="w-full text-sm p-2 px-3 border border-slate-200 rounded outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">
                </div>
            </div>
            ...
            <div class="space-y-1">
                ...
                <textarea name="address" id="address" rows="3" placeholder="Street, City, District"
                    class="w-full text-sm p-2 px-3 border border-slate-200 rounded outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all">{{ old('address', $supplier->address) }}</textarea>
            </div>
        </div>

        <div class="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
            <a href="{{ route('admin.suppliers.index') }}" class="bg-white border border-slate-200 hover:bg-slate-100 text-slate-600 font-semibold text-xs px-4 py-2 rounded transition-colors uppercase tracking-wide flex items-center justify-center">Cancel</a>
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-5 py-2 rounded shadow-xs transition-colors uppercase tracking-wide flex items-center gap-1.5">
                <i class="fa-solid fa-rotate"></i> Update Supplier Record
            </button>
        </div>
    </form>
</div>
